...
public class FNS3551FLClient implements Client {

	static Logger log = Logger.getLogger(FNS3551FLClient.class.getName());
	...
	public ClientRequest createClientRequest(ExchangeContext ctx) {
		...
		if (documentRequest.getRequestType() == DocumentRequestType.ЗАПРОС_ДОКУМЕНТА) {
			clientRequest.action = new QName("http://ws.unisoft/", "SendFullFLRequest");
			packet.status = Packet.Status.REQUEST;
		}
		if (documentRequest.getRequestType() == DocumentRequestType.ПРОВЕРКА_ВЫПОЛНЕНИЯ) {
			clientRequest.action = new QName("http://ws.unisoft/", "GetFullFLResponse");
			packet.status = Packet.Status.REQUEST;
		}
	...
	private String createAppData(ExchangeContext ctx, DocumentRequest documentRequest) {
		...
		try {
			...
			if (documentRequest.getRequestType() == DocumentRequestType.ЗАПРОС_ДОКУМЕНТА) {
				unisoft.ws.egrnxx.fullflreq.Документ doc = new unisoft.ws.egrnxx.fullflreq.Документ();
				doc.setВерсФорм("4.02");
				doc.setНомерДела("01");
				doc.setИдДок(UUID.randomUUID().toString());
				
				unisoft.ws.egrnxx.fullflreq.Документ.ЗапросИП запрос = new unisoft.ws.egrnxx.fullflreq.Документ.ЗапросИП();
				запрос.setИНН((String) params.get("ИНН"));
				запрос.setОГРНИП((String) params.get("ОГРНИП"));
				doc.setЗапросИП(запрос);
				
			    JAXBContext jaxbContext = JAXBContext.newInstance(unisoft.ws.egrnxx.fullflreq.Документ.class);
	...
	public void processClientResponse(ClientResponse clientResponse, ExchangeContext ctx) {
		...
		if (documentRequest.getRequestType() == DocumentRequestType.ПРОВЕРКА_ВЫПОЛНЕНИЯ) {
			try {
				...
				JAXBContext jaxbContext = JAXBContext.newInstance(unisoft.ws.egrnxx.responsevipfl.Документ.class);
				JAXBElement<unisoft.ws.egrnxx.responsevipfl.Документ> appDataElement = jaxbContext.createUnmarshaller().unmarshal(docEl, unisoft.ws.egrnxx.responsevipfl.Документ.class);
				unisoft.ws.egrnxx.responsevipfl.Документ appDataDoc = appDataElement.getValue();
				...
				if ("99".equals(appDataDoc.getКодОбр())) {
					...
					if (appDataDoc.getСвИП() != null) {
						...
						doc.setDocumentType(ТипДокумента.ВЫПИСКА_ИЗ_ЕГРИП_ПОЛНАЯ);
...
//
	...
	public static SOAPMessage readSoapMessage(String resourceName) throws Exception {
		...
		InputStream is = FNS3551FLClient.class.getClassLoader().getResourceAsStream(resourceName);
	...
	public static void main(String[] args) throws Exception {
		FNS3551FLClient client = new FNS3551FLClient();
		...
		dr.setRequestParameter("ОГРНИП", "304301728800074");
