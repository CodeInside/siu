...
public abstract class AbstractDatabase implements Database {
    ...
    public void setConnection(DatabaseConnection conn) {
        logger.fine("Connected to " + conn.getConnectionUserName() + "@" + conn.getURL());
        this.connection = conn;
        try {
            connection.setAutoCommit(getAutoCommitMode());
        } catch (DatabaseException sqle) {
            logger.warning("Can not set auto commit to " + getAutoCommitMode() + " on connection");
        }
    }
...
// ------- DATABASECHANGELOG / DATABASECHANGELOGLOCK METHODS ---- //
    ...
    public void checkDatabaseChangeLogTable(boolean updateExistingNullChecksums, DatabaseChangeLog databaseChangeLog, String... contexts) throws DatabaseException {
        ...
        if (changeLogTable != null) {
            ...
            logger.fine("Creating database history table with name: " + escapeTableName(getDefaultSchemaName(), getDatabaseChangeLogTableName()));
//                }
        }
            ...
            for (RanChangeSet ranChangeSet : this.getRanChangeSetList()) {
                if (ranChangeSet.getLastCheckSum() == null) {
                    ...
                    if (changeSet != null && new ContextChangeSetFilter(contexts).accepts(changeSet) && new DbmsChangeSetFilter(this).accepts(changeSet)) {
                        logger.info("Updating null or out of date checksum on changeSet " + changeSet + " to correct value");
    ...
    public void checkDatabaseChangeLogLockTable() throws DatabaseException {
        ...
        if (!hasDatabaseChangeLogLockTable()) {
            ...
            this.commit();
            logger.fine("Created database lock table with name: " + escapeTableName(getLiquibaseSchemaName(), getDatabaseChangeLogLockTableName()));
    ...
    public ChangeSet.RunStatus getRunStatus(ChangeSet changeSet) throws DatabaseException, DatabaseHistoryException {
        ...
        if (foundRan == null) {
            ...
            if (foundRan.getLastCheckSum() == null) {
                try {
                    logger.info("Updating NULL md5sum for " + changeSet.toString());
...
//                        throw new DatabaseHistoryException("MD5 Check for " + changeSet.toString() + " failed");
    ...
    public List<RanChangeSet> getRanChangeSetList() throws DatabaseException {
        ...
        if (hasDatabaseChangeLogTable()) {
            logger.fine("Reading from " + databaseChangeLogTableName);
            ...
            for (Map rs : results) {
                ...
                try {
                    ...
                    logger.severe("Unknown EXECTYPE from database: " + execType);
                    throw e;
                }
            }
        }
