...
public class ChangeSet implements Conditional {
    ...
    public CheckSum generateCheckSum() {
        final StringBuffer stringToMD5 = new StringBuffer();
        for (Change change : getChanges()) {
            stringToMD5.append(change.generateCheckSum()).append(":");
        }
        for (SqlVisitor visitor : getSqlVisitors()) {
        ...
        try {
            ...
            if (!skipChange) {
                ...
                for (Change change : getChanges()) {
                    ...
                    log.fine(change.getConfirmationMessage());
                }
            ...
            if (getFailOnError() != null && !getFailOnError()) {
                ...
                log.log(Level.FINE, "Failure Stacktrace", e);
                execType = ExecType.FAILED;
            } else {
                log.log(Level.SEVERE, "Change Set " + toString(false) + " failed.  Error: " + e.getMessage(), e);
    ...
    public void rollback(Database database) throws RollbackFailedException {
        try {
            ...
            database.commit();
            log.fine("ChangeSet " + toString() + " has been successfully rolled back.");
        } catch (Exception e) {
            try {
                database.rollback();
